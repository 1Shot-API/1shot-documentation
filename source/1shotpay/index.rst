...
There is no KYC or AML required to use 1ShotPay. Sign up on mobile or desktop in a few seconds by claiming a unique username and optionally backing up your account. You can fund you account from your USDC balance on Base network using any external Ethereum-compatible wallet or onramp with Coinbase Onramp with 0 fees with your Coinbase account by clicking the "Deposit" button on the home screen.

Any web application (including onchain apps built on 1Shot API) can leverage 1ShotPay as an meta-onboarding provider to enable users to sign up, fund accounts, checkout or pay for services without the need for expensive payment processors or waiting to get approved by onromp providers. 
